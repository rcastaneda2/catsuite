...
tabItem_ATA_plot_output = tabItem(
  ...
  fluidRow(
    box(
      ...
      plotlyOutput("three_in_1_IRT_plot", width = "100%")
  ...
  fluidRow(
    box(
      title = "Download", status = "primary", solidHeader = T, collapsible = T, collapsed = T, width = 6,

      checkboxGroupInput(inputId = "download_checkbox", label = "Download",
                         choices = list(modules = "Modules", rds = "Rds"),
                         inline = T)
    )
  )
)
