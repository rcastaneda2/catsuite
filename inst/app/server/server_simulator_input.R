...
# -----------------------------------
#
# 1. load simulation examples from CATSimulator
#
# for debugging
# available_simulations = sort(gsub("^(.*).rds$", "\\1", dir(system.file("example", package = "CATSimulator"))))
# sysFile = system.file("example/gpc3pl-adaptive-maxinf.rds", package = "CATSimulator")
# simulation = readRDS(sysFile)
# rea_simulator = list(); rea_simulator$latest <- simulation
...
# 2. load simulation rds from local drive
...
# functions used for Simulation Input
...
syncTestConfigUI <- function(simulation) {
  ...
  testName = ifelse(is.null(simulation$testName), "My Test", simulation$testName)
  updateTextInput(session, "simulation_test_name", value = testName)
...
updateLatestFromUI = function () {
  ...
  rea_simulator$latest$testName                 <- input$simulation_test_name
  ...
  tryCatch({
    withProgress(value = 0.1, message = "Simulation preparing...", detail = "", {
      isolate({
        ...
        # create a simulee generator from the Simulator Config settings

        if (input$simulation_generator == "normal") {
          simuleeGenerator = CATSimulator::generateSimuleesByNormal(
            numSimulees = input$simulation_simulees_slider,
            mean = input$simulation_mean_slider,
            sd = input$simulation_stddev_slider,
            randomSeeds = input$simulation_seed_slider:(input$simulation_seed_slider + input$simulation_simulees_slider - 1)
          )
        } else {
          ...
          simuleeGenerator = CATSimulator::generateSimuleesByTrueTheta(
            trueThetas = rep_len(trueThetas, input$simulation_simulees_slider),
            randomSeeds = input$simulation_seed_slider:(input$simulation_seed_slider + input$simulation_simulees_slider - 1)
          )
        }
        ...
        rea_simulator$latest$label <- paste0(rea_simulator$latest$testName, "-", format(rea_simulator$latest$timestamp, DATETIME_FORMAT))
        rea_simulator$latest$result <- CATSimulator::runSimulation(rea_simulator$latest, simuleeGenerator, function(completedCounter) {
          if ((completedCounter %% 10) == 0) {
            pctComplete = completedCounter / input$simulation_simulees_slider
            detail = paste0(completedCounter, "/", input$simulation_simulees_slider)
...
output$test_groups_table <- renderDT({
  if (is.null(rea_simulator$latest) | is.null(rea_simulator$latest$groups)) return ()

  data = data.frame(
    GROUP_ID = rea_simulator$latest$groups$GROUP_ID,
    ITEM_IDS = vapply(rea_simulator$latest$groups$ITEM_IDS, function(groupItemIds) {
      paste(groupItemIds, collapse=", ")
    }, as.character(0), USE.NAMES = FALSE)
  )

  if ("PSG_ID" %in% names(rea_simulator$latest$groups)) {
    data = add_column(data, .after = "GROUP_ID",
      PSG_ID = rea_simulator$latest$groups$PSG_ID
    )
  }
  if ("STAGES" %in% names(rea_simulator$latest$groups)) {
    data = add_column(data, .after = "GROUP_ID",
      STAGES = vapply(rea_simulator$latest$groups$STAGES, function(groupStages) {
...
output$pool_cons_histogram <- renderPlotly({
  ...
  # saveRDS(itempool, file = "itempool.rds")
  # itempool = readRDS(file = "/Users/may/myProgram/MST/catsuite/catsuite/itempool.rds")

  contentCons = rea_simulator$latest$constraints$content
  # transform(contentCons, CONS_ID = as.character(CONS_ID))
  ...
  print(sapply(contentCons, class))
  print(sapply(cons_freq, class))
  ...
  p <- plot_ly(cons_tbl, x = ~ CONS_ID, y = ~value,
               color = ~ key,
               colors = LOWER_FREQ_UPPER,
               # group = ~key,
               # marker = list(color = colors[match(cons_tbl$key, names(colors))]),
               type = "bar",
               opacity = '.7')
