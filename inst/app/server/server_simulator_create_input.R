...
# --------------------------------
#       Create CAT Constraints
# --------------------------------

# call module gSheetFile to load google sheets 
# the returning datafile() is reactive. 

# selPoolFile <- callModule(gSheetFile, "load_sim_pool_gh_file")  
...
# reder UI to show all google sheets or loaded examples, where one tabPanel with one DT is for one sheet
...
# List how to convert attributes to constraints
...
# selectInput("cate_attr_sel", "select attribute:", choices = 1),
# uiOutput("cate_attr_radiobox_UI"),
# dataTableOutput("attr_to_cons_holder")

# observeEvent(selPoolFile(), { 
#   
#   cat("### ", format(Sys.time(), "%X"), " observeEvent: selPoolFile()\n")
#   
#   if (is.null(selPoolFile)) return()
#   data = selPoolFile()[[1]]
#   
#   rea_self_input$attr_to_cons_holder = NULL
#   rea_self_input$itempool = selPoolFile()[[1]]
# 
# })

# for debugging 
# data <- read.csv("~/myProgram/MST/NCME_2019_workshop/data/pool_3PL.csv")
...
# ---- Action--add categorical attribute to constraint
...
# ---- Action--add numeric attribute to constraint
...
# add constraints to rea_self_input$itempool based on rea_self_input$attr_to_cons_holder
...
create_blueprint = function (attr_to_cons_holder) {
  ...
  # attr_to_cons_holder = read_rds("/Users/may/myProgram/MST/catsuite/catsuite/attr_to_cons_holder.rds")
  # > attr_to_cons_holder
  # # A tibble: 4 x 6
  # CONS_ID ATTRIBUTE            ATTRIBUTE_VALUE RANGE_FROM RANGE_TO LABEL                      
  # <chr>   <chr>                <chr>                     <dbl>           <dbl> <chr>                      
  # 1 CONS1   COGNITIVE_COMPLEXITY medium                   NA               NA    COGNITIVE_COMPLEXITY-medium
  # 2 CONS2   COGNITIVE_COMPLEXITY high                     NA               NA    COGNITIVE_COMPLEXITY-high  
  # 3 CONS3   EXPOSURE_RATE        NA                        0.02             0.06 EXPOSURE_RATE-0.02 ~ 0.06  
  # 4 CONS4   EXPOSURE_RATE        NA                        0.385            0.42 EXPOSURE_RATE-0.385 ~ 0.42 
  
  
  # sysFile = system.file("example/shallow-adaptive-wpm.rds", package = "CATSimulator")
  # simulation = readRDS(sysFile)
  
  # > simulation$constraints
  # # A tibble: 13 x 5
  # CONS_ID WEIGHT LOWER UPPER LABEL
  # <chr>    <dbl> <dbl> <dbl> <chr>
  # 1 CONS01     100  0.1   0.15 1A   
  # 2 CONS02     100  0.1   0.15 1B   
  # 3 CONS03     100  0.1   0.15 1C   
  # 4 CONS04     100  0.3   0.35 RC1  
  # 5 CONS05     100  0.1   0.15 2A   
...
createInputToSimulation = function (test_name, constraints, itempool) {
  ...
  # itempool = read_rds("/Users/may/myProgram/MST/catsuite/catsuite/itempool.rds")
  # constraints = read_rds("/Users/may/myProgram/MST/catsuite/catsuite/constraints.rds")
  ...
  simulation$itempool = tibble(
    ...
    ## PAR_4 = itempool$IRT_PAR_D2,
    ## PAR_5 = itempool$IRT_PAR_D3,
    ## PAR_6 = itempool$IRT_PAR_D4,
    ## PAR_7 = itempool$IRT_PAR_D5,
    ## PAR_8 = itempool$IRT_PAR_D6,
    ## PAR_9 = itempool$IRT_PAR_D7,
  ...
  # write_rds(simulation2, path = "simulation2.rds")
  # simulation2 = read_rds("/Users/may/myProgram/MST/catsuite/catsuite/simulation2.rds")
  return(simulation)
}

# call module editDT to show the google sheets and assign those sheets to rea_input$tibbles
# not used now
# observeEvent(selPoolFile(), { 
#   
#   cat("### ", format(Sys.time(), "%X"), " **************** observeEvent: selPoolFile()\n")
#   
#   # print(head(datafile()))  #ok get a list of tibble--POOL, SLICE, ALIAS, TASK
#   
#   # saveRDS(isolate(datafile()), file = "data.rds")
#   
#   # for create input in simulator
#   # rea_self_input = reactiveValues(pool = NULL)
#   
#   rea_self_input$itempool <- callModule(editDT, "selPoolEditTable", data_sets = isolate(selPoolFile()))
#   
#   print(head(rea_self_input$itempool()))
#   
# })
