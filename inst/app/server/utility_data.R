...
# getCateCols = function (data) {
#   
#   
#   cateCols = names(data)[setdiff(which(unlist(map(data, is.categorical))),
#                                  {which(map(data,is.na) %>% map(all) %>% unlist())})]
#   
#   if (any(grep("^IRT_PAR_D|^IRT_PAR_C|^IRT_SC", cateCols)))
#     cateCols = cateCols[-grep("^IRT_PAR_D|^IRT_PAR_C|^IRT_SC", cateCols)] 
#   
#   if ("EVIDENCE_STATEMENT" %in% names(data))
#     cateCols = c(cateCols, "EVIDENCE_STATEMENT")
#   
#   return(cateCols)
# }
...
# currently, this funciton is very dummy. 
